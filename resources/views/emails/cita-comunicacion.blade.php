...
<head>
    <meta charset="UTF-8">
    <title>Comunicación de Cita</title>
</head>
...
<body style="margin:0; padding:25px; font-family: Arial, sans-serif; background-color: #f4f7f6;">
    <div
        ...
        <div
            ...
            <h4 style="margin-top: 0; color: #02424e; font-size: 15px;">Referencia de la Cita:</h4>
            <ul style="font-size:14px; line-height:1.6; list-style: none; padding-left: 0; margin-bottom: 0;">
                <li><strong>Cita Nro:</strong> #{{ $cita->id }}</li>
                <li><strong>Empresa:</strong> {{ $cita->unidadNegocio->nombre ?? 'N/A' }}</li>
                <li><strong>Proyecto:</strong> {{ $cita->proyecto->nombre ?? 'N/A' }}</li>
                <li><strong>Sede:</strong> {{ $cita->sede->nombre ?? 'N/A' }}</li>
                <li><strong>Fecha:</strong> {{ $cita->fecha_inicio?->format('d/m/Y H:i') ?? 'N/A' }}</li>
                <li><strong>Estado actual:</strong> {{ $cita->estado->nombre ?? 'N/A' }}</li>
            </ul>
        </div>
