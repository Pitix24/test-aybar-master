<div class="g_gap_pagina">
    <div class="g_panel cabecera_titulo_pagina">
        <h2>Editar Cliente Antiguo (DB2)</h2>
    ...
    <div class="g_fila">
        <div class="g_columna_8 g_gap_pagina">
            <div class="g_panel">
                <h4 class="g_panel_titulo">Registros Asociados al DNI: {{ $dni }}</h4>
                        ...
                        <table class="g_tabla g_tabla_pequena">
                            ...
                            <tbody>
                                @foreach ($informaciones as $index => $informacion)
                                    <tr class="{{ $informacion->id == $registro->id ? 'g_fila_activa' : '' }}">
                                        <td> {{ $index + 1 }} </td>
                                        <td class="g_negrita g_celda_wrap">{{ $informacion->razon_social }}</td>
                                        <td><span class="g_badge g_badge_light">{{ $informacion->proyecto }}</span></td>
        ...
        <div class="g_columna_4">
            <form wire:submit="update" class="g_panel formulario">
                <h4 class="g_panel_titulo">Editar Información</h4>

                <div class="g_margin_bottom_10">
                    <label for="dni">DNI / CE / RUC <span class="obligatorio"><i
                                class="fa-solid fa-asterisk"></i></span></label>
                    <input type="text" id="dni" wire:model="dni" class="@error('dni') input-error @enderror"
                        autocomplete="off">
                    @error('dni')
                ...
                <div class="formulario_botones">
                    <button type="submit" class="g_boton guardar g_columna_12" wire:loading.attr="disabled">
                        <span wire:loading.remove wire:target="update">
                            <i class="fa-solid fa-save"></i> Actualizar Registro
                        </span>
