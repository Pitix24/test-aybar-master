    ...
    <div class="g_panel">
        <div class="g_fila" style="gap:10px;">
            <div class="g_columna_6">
                <input type="text" wire:model.live="buscar" placeholder="Buscar tarea o instruccion...">
            </div>
                    ...
                    @forelse($items as $t)
                        <tr wire:key="tarea-{{ $t->id }}">
                            ...
                            <td>
                                ...
                                <p class="g_inferior" style="margin:0; font-size:11px;">{{ Str::limit($t->instruccion, 60) }}</p>
                            </td>
                            <td>
                                <span class="g_badge {{ $t->fase === 'ANTES' ? 'info' : ($t->fase === 'DURANTE' ? 'warning' : 'success') }} g_mayuscula" style="font-size:11px;">
                                    {{ $t->fase }}
                                </span>
                            </td>
                            <td class="g_celda_centro">
                                @if($t->esta_completado)
                                    <span class="g_badge success" style="font-size:11px;"><i class="fa-solid fa-check"></i> Completada</span>
                                @else
                                    <span class="g_badge light" style="font-size:11px;">Pendiente</span>
                                @endif
                            </td>
                            <td class="g_celda_acciones g_celda_centro">
                                <a href="{{ route('erp.entrega-fest.mop.tareas.editar', [$evento->id, $t->id]) }}" class="g_accion editar" title="Editar">
