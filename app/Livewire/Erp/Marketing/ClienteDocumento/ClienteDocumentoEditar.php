...
class ClienteDocumentoEditar extends Component
{
    ...
    {
        $this->documento = ClienteDocumento::with('archivoPdf')->findOrFail($id);

        $this->proyecto_id = $this->documento->proyecto_id;
        $this->tipo_cliente_documentos_id = $this->documento->tipo_cliente_documentos_id;
        $this->titulo = $this->documento->titulo;
        $this->descripcion = $this->documento->descripcion;
        $this->icono = $this->documento->icono;
        $this->orden = $this->documento->orden;
        $this->solo_lectura = $this->documento->solo_lectura;
        $this->activo = $this->documento->activo;
    }
        ...
        try {
            ...
            if ($this->archivo_nuevo) {
                ...
                $this->documento->archivoPdf()->create([
                    ...
                    'path' => $path,
                    'url' => route('cliente.documentos.stream', ['id' => $this->documento->id]),
    ...
    public function render()
    {
        ...
        return view('livewire.erp.marketing.cliente-documento.cliente-documento-editar', [
            'proyectos' => $proyectos,
            'tipos' => $tipos,
        ]);
    }
