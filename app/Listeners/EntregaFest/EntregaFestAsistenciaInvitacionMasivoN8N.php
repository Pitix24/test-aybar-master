...
class EntregaFestAsistenciaInvitacionMasivoN8N
{
     ...
     * Handle the event.
     */
    public function handle(EntregaFestAsistenciaInvitacionMasivo $event): void
    {
        $evento = $event->evento;
        ...
        $contactos = ProspectoEntregaFest::where('entrega_fest_id', $evento->id)
            ...
            ->map(function (ProspectoEntregaFest $prospecto) use ($plantilla, $etapa) {
                ...
                $mailPropietario = new AsistenciaInvitacionPropietarioMail($prospecto, $plantilla);
                ...
                return [
                    ...
                    'enviar_email' => is_null($prospecto->invitacion_confirmada) && !$yaFueEmail,
                    'enviar_whatsapp' => is_null($prospecto->invitacion_confirmada) && !$yaFueWhatsapp,
                    'tipo' => 'Propietario',

                    'copropietarios' => $prospecto->copropietarios
                        ->whereNull('invitacion_confirmada')
                        ->unique('dni') // Solo un envío por DNI de Copropietario
                        ->map(function ($copro) use ($plantilla, $etapa) {
                            ...
                            $mailCopro = new AsistenciaInvitacionCopropietarioMail($copro, $plantilla);
        ...
        if (empty($contactos)) {
            Log::channel('entrega-fest')->warning("[ASISTENCIA-INVITACION-MASIVO-N8N] No hay contactos para el evento #{$evento->id}");
            return;
        }
