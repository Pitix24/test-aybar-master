...
INSERT INTO aybar.grupo_proyectos (
        id,
        nombre,
        activo,
        created_at,
...
SELECT id,
    ...
    updated_at,
    deleted_at
FROM aybarcorp2.grupo_proyectos;
...
INSERT INTO aybar.area_tipo_solicitud (
        id,
        area_id,
        tipo_solicitud_id,
        created_at,
...
SELECT ats.id,
    ats.area_id,
    ats.tipo_solicitud_id,
    ats.created_at,
    ats.updated_at
FROM aybarcorp.area_tipo_solicitud ats
    LEFT JOIN aybar.area_tipo_solicitud a2 ON a2.id = ats.id
WHERE a2.id IS NULL;
INSERT INTO aybar.estado_tickets (
        id,
        nombre,
        color,
        icono,
...
SELECT a.id,
    ...
    1 AS user_id,
    -- ADMIN / SISTEMA
    COALESCE(a.titulo, a.path) AS nombre_original,
    a.path,
    a.url,
    a.titulo,
    a.descripcion,
    COALESCE(a.extension, '') AS extension,
    0 AS size,
...
INSERT INTO aybar.ticket_historials (
        id,
        ticket_id,
        user_id,
        accion,
...
CREATE TABLE aybar.clientes_2 LIKE aybarcorp2.clientes_2;
INSERT INTO aybar.clientes_2
SELECT *
FROM aybarcorp2.clientes_2;
INSERT INTO aybar.motivo_citas (
        id,
        nombre,
        activo,
        created_at,
...
SELECT c.id,
    ...
    c.updated_at,
    c.deleted_at
FROM aybarcorp.citas c
    LEFT JOIN aybar.citas cc ON cc.id = c.id
WHERE cc.id IS NULL;
INSERT INTO aybar.estado_evidencia_pagos (
        id,
        nombre,
        color,
        icono,
...
SELECT s2.id,
    s2.unidad_negocio_id,
    s2.proyecto_id,
    s2.cliente_id,
    s2.gestor_id,
    s2.estado_evidencia_pago_id,
    s2.lote_completo,
    s2.codigo_cuota,
    s2.razon_social,
    s2.nombre_proyecto,
    ...
    CASE
        WHEN CAST(s2.fecha_validacion AS CHAR) = '0000-00-00 00:00:00' THEN NULL
        ELSE s2.fecha_validacion
    END,
    s2.created_by,
    s2.updated_by,
    s2.deleted_by,
...
SELECT e2.id,
    e2.solicitud_evidencia_pago_id,
    e2.estado_evidencia_pago_id,
    e2.path,
    e2.url,
    e2.extension,
    e2.numero_operacion,
...
FROM aybarcorp2.evidencia_pagos e2
    ...
    INNER JOIN aybar.solicitud_evidencia_pagos s ON s.id = e2.solicitud_evidencia_pago_id -- valida que exista
WHERE e1.id IS NULL;
...
FROM aybarcorp2.correo_evidencia_pagos c2
    ...
    INNER JOIN aybar.solicitud_evidencia_pagos s ON s.id = c2.solicitud_evidencia_pago_id -- valida FK
WHERE c1.id IS NULL;
...
FROM aybarcorp2.evidencia_pago_antiguos e2
    LEFT JOIN aybar.evidencia_pago_antiguos e1 ON e1.id = e2.id -- Validación de FKs para evitar errores
    LEFT JOIN aybar.unidad_negocios un ON un.id = e2.unidad_negocio_id
    LEFT JOIN aybar.proyectos p ON p.id = e2.proyecto_id
    LEFT JOIN aybar.users u1 ON u1.id = e2.cliente_id
    LEFT JOIN aybar.users u2 ON u2.id = e2.gestor_id
